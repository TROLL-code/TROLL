...
float total_daily_light, phitdl;
...
void Initialise() {
    ...
    if (In) {
        ...
        for (int i=0; i<=23; i++) {
            In >> daily_light[i];
            total_daily_light += daily_light[i];
        }
        cerr << "total_daily_light " << total_daily_light << endl;
...
void UpdateTree() {
    ...
    //if(iter == 180) for(int i=0;i<100;i++) cerr << i << "\t" << histalpha[i] <<endl;
