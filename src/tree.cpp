...
void Tree::Birth(Context &ctx, int nume, int site0)
{
    ...
    if (ctx.field.LAI3D[0][site0 + ctx.grid.SBORD] < LAImax_precomputed)
    {
        ...
        if (t_g1 < 0)
        {
            LOG_COUT << "Warning: t_g1 <0 in Tree::Birth ! t_LMA=" << t_LMA << "; s_LMA=" << ctx.S[t_sp_lab].s_LMA << "; multiplierLMA=" << t_mult_LMA << endl;
        }
#endif
        ...
        if (ctx.opt._LA_regulation > 0)
        {
            ...
            // CalcLAImax(ctx);
            // LOG_COUT << "LAImax: " << t_LAImax << " precomputed: " << LAImax_precomputed << endl; // check consistency
#ifdef LCP_alternative
...
int Tree::BirthFromInventory(Context &ctx, int site, vector<string> &parameter_names, vector<string> &parameter_values, int &nb_speciesrandom)
{
    ...
    if (t_dbh > 0.0)
    {
        ...
        if (species_exists == 0)
        {
            ...
            nb_speciesrandom++;
            // LOG_COUT << "Species: " << parameter_value << " not found. Initializing as random species: " << ctx.S[t_sp_lab].s_name << endl;
        }
...
void Tree::Water_availability(Context &ctx)
{
    ...
    for (int l = 0; l < ctx.soil.nblayers_soil; l++)
    {
        ...
        if (t_LA > 0.0)
        {
            ...
            if (isnan(t_phi_root))
            {
                LOG_COUT << "nan t_phi_root: " << endl;
                LOG_COUT << l << "\t" << t_soil_layer_weight[l] << "\t" << ctx.soil.KsPhi[l][ctx.grid.site_DCELL[t_site]] << ctx.soil.soil_phi3D[l][ctx.grid.site_DCELL[t_site]] << "\t" << endl;
            }
        }
        else
        {
        ...
        // if(t_soil_layer_weight[l]<=0) LOG_COUT << "t_soil_layer_weight[l]=" << t_soil_layer_weight[l] << " t_root_biomass[l]=" << t_root_biomass[l] << " ctx.soil.Ks=" << ctx.soil.Ks[l][ctx.grid.site_DCELL[t_site]] << " ctx.soil.Ks*Phi soil =" << ctx.soil.KsPhi[l][ctx.grid.site_DCELL[t_site]] << " ctx.params.phi soil=" <<   ctx.soil.soil_phi3D[l][ctx.grid.site_DCELL[t_site]] << endl;
        ...
        {
            LOG_COUT << "incorrect soil_layer_weight: " << endl;
            LOG_COUT << l << "\t" << ctx.grid.site_DCELL[t_site] << "\t" << t_soil_layer_weight[l] << "\t" << t_root_biomass[l] << "\t" << ctx.soil.Ks[l][ctx.grid.site_DCELL[t_site]] << "\t" << ctx.soil.KsPhi[l][ctx.grid.site_DCELL[t_site]] << "\t" << -log(sqrt(PI * t_root_biomass[l] * 10) * 0.001) << "\t" << deep_bound << "\t" << shallow_bound << "\t" << ctx.soil.SWC3D[l][ctx.grid.site_DCELL[t_site]] << "\t" << (ctx.soil.SWC3D[l][ctx.grid.site_DCELL[t_site]] - ctx.soil.Min_SWC[l]) / (ctx.soil.Max_SWC[l] - ctx.soil.Min_SWC[l]) << "\t" << ctx.soil.Ksat[l] << "\t" << ctx.soil.soil_phi3D[l][ctx.grid.site_DCELL[t_site]];
            LOG_COUT << endl;
        }
    }
    ...
    else
    {
        ...
        for (int l = 0; l < ctx.soil.nblayers_soil; l++)
        {
            ...
            {
                LOG_COUT << "nan t_phi_root" << endl;
                LOG_COUT << t_root_biomass[l] << "\t" << ctx.soil.soil_phi3D[l][ctx.grid.site_DCELL[t_site]] << endl;
            }
    ...
    if (sum_weight > 1.5)
    {
        LOG_COUT << "Weird weights in Water_availability; sum_weight=" << sum_weight << " sumG=" << sumG << " total_root_biomass=" << total_root_biomass << " t_phi_root=" << t_phi_root;
        if (sumG > 0.0)
        {
            float isumG = 1.0 / sumG;
            LOG_COUT << " isumG=" << isumG << endl;
        }
        LOG_COUT << "t_soil_layer_weight[l]: ";
        for (int l = 0; l < ctx.soil.nblayers_soil; l++)
        {
            LOG_COUT << t_soil_layer_weight[l] << "\t";
        }
        ...
        for (int l = 0; l < ctx.soil.nblayers_soil; l++)
        {
            LOG_COUT << ctx.soil.soil_phi3D[l][ctx.grid.site_DCELL[t_site]] << "\t";
        }
        LOG_COUT << endl;
    }
    ...
    {
        LOG_COUT << "incorrect value in one of WSF, WSF_A, t_phi_root " << endl;
        LOG_COUT << t_WSF << "\t" << t_phi_root << "\t" << t_tlp << "\t" << t_dbh << "\t" << t_height << "\t" << t_age << "\t" << t_phi_lethal << "\t" << sumG;
        LOG_COUT << endl;
    }
    //}
}
...
void Tree::Water_uptake(Context &ctx)
{
    if (t_age > 0.0)
    {
        ...
        while (sum_weights < 1.0 && l < ctx.soil.nblayers_soil)
        {
            ...
            {
                LOG_COUT << setprecision(10);
                LOG_COUT << "Problem with soil_layer_weight and transpiration at site: " << t_site << " ctx.diag.layer: " << l << " depth: " << depth << endl;
                LOG_COUT << t_soil_layer_weight[l] << "\t" << t_transpiration << "\n";
            }
...
void Tree::Fluxh(Context &ctx, int h, float &PPFD, float &VPD, float &Tmp, float &leafarea_layer)
{
    ...
    // if (PPFD <=0) {
    //  LOG_COUT << "Warning PPFD <=0 in Fluxh !! PPFD=" << PPFD << "; iLA_layer=" << iLA_layer << "; canopy_environment_cumulated[1]=" << canopy_environment_cumulated[1] << "; VPD=" << VPD << "; Tmp=" << Tmp << "; h=" << h << "; Tree height=" << t_height << "; Tree_crowndepth=" << t_CD << "; Tree_leafarea=" << t_LA << endl;
    //}
}
...
float Tree::DeathRate(Context &ctx, float dbh, float carbon_starv, float phi_root)
{
    ...
    /*if (ctx.time.iter>=622 && dr*ctx.time.timestep>=0.8) {
        LOG_COUT<< "high deathrate: wsg=" << t_wsg << "; basal=" << basal << "; dbh="  << dbh << "; dr="  << dr*ctx.time.timestep   << "; carbon_starv="  << carbon_starv   << "; NPP="  << t_NPP   << "; phi_root="  << phi_root   << "; ctx.S[t_sp_lab].s_phi_lethal=" << ctx.S[t_sp_lab].s_phi_lethal << "; t_WSF=" << t_WSF << "; t_WSF_A=" << t_WSF_A << "; t_LA=" << t_LA << endl;
    }*/
...
leafFluxes Tree::FluxesLeaf(Context &ctx, float PPFD, float VPDa, float Ta, float WIND, float ExtinctLW, float PPFDtop, float PPFDinc, float Tatop, float VPDatop)
{
    ...
    // LOG_COUT << "PPFDtop=" << PPFDtop << " PPFD=" << PPFD <<" PPFDinc=" << PPFDinc << " RSOL=" << RSOL << " RSOL_NIR=" << RSOL_NIR << endl;
    // }
    ...
    // if (ExtinctLW<0.2) {
    //    LOG_COUT << "Rnetiso=" << Rnetiso << " RSOL=" << RSOL << " INLR=" << ctx.lookup.LookUp_INLR[convTA][convVPDA]  << " INLR_top=" << ctx.lookup.LookUp_INLR[convTAtop][convVPDAtop] << " //ExtinctLW=" << ExtinctLW <<  endl;
    // }
    ...
    { // Iterative loop:
        ...
        {
            LOG_COUT << "Warning in FluxesLeaf ! DS=" << DS << "; CS=" << CS << "; PPFD=" << PPFD << "; TLEAF=" << TLEAF << endl;
        }
        ...
        {
            LOG_COUT << "Warning in FluxesLeaf: " << " ALEAF= " << ALEAF << ", GSC= " << GSC << ", CS= " << CS << ", PPFD=" << PPFD << ", DS= " << DS << ", TLEAF= " << TLEAF << ", ITER=" << ITER << endl;
        }
        ...
        {
            LOG_COUT << "Warning in FluxesLeaf: " << " GV= " << GV << ", GSC= " << GSC << ", GBV= " << GBV << ", GSV=" << GSV << ", GSC=" << GSC << ", ALEAF=" << ALEAF << ", GSVGSC= " << GSVGSC << ", ALEAF=" << ALEAF << ", PPFD=" << PPFD << ", DS=" << DS << ", ITER=" << ITER << ", WIND=" << WIND << endl;
        }
        ...
        if (CS <= 0)
        {
            LOG_COUT << "Warning in FluxesLeaf: " << " CS= " << CS << ", Cair=" << ctx.params.Cair << ", ALEAF=" << ALEAF << ", GBH=" << GBH << ", GBHGBC=" << GBHGBC << endl;
        }

        /*if(DS < 0 ) {

            LOG_COUT << "Warning in FluxesLeaf: " << " DS= " << DS << ", VPDa= "<< VPDa << ", ITER=" << ITER << ", GSV=" << GSV << ", GSC=" << GSC << ", ALEAF=" << ALEAF << ", ET= " << ET << ", GV=" << GV << ", initial lambdaET=" << (SLOPE*Rnetiso+1000.0*VPDa*HDIVT)/(SLOPE+GAMMA*GH/GV) << ", Rnetiso=" << Rnetiso << "; PPFD=" << PPFD << "; Vcmax=" << t_Vcmax << "; Jmax=" << t_Jmax << "; t_Pmass=" << t_Pmass << "; ctx.S[spp].s_Pmass=" << ctx.S[t_sp_lab].s_Pmass << endl ;
        }*/
        ...
        {
            LOG_COUT << "Warning in FluxesLeaf: " << " ALEAF= " << ALEAF << ", ET= " << ET << ", CS= " << CS << ", PPFD=" << PPFD << ", DS= " << DS << ", TLEAF= " << TLEAF << ", GSV=" << GSV << ", GV= " << GV << ", Rnetsiso=" << Rnetiso << "; lambdaET=" << lambdaET << "; HDIVT=" << HDIVT << "; TDIFF=" << TDIFF << "; Ta=" << Ta << "; Cair=" << ctx.params.Cair << "; GBH=" << GBH << "; Cair=" << ctx.params.Cair << "; GBHGBC=" << GBHGBC << "; t_site=" << t_site << "; t_sp_lab=" << t_sp_lab << "; ITER=" << ITER << endl;
        }
    }

    // LOG_COUT << "itmax=" << itmax << endl;
    ...
    //     LOG_COUT << " species:" << ctx.S[t_sp_lab].s_name << " Tree height=" << t_height << " ET=" << ET << " SLOPE=" << SLOPE << " Rnetiso=" << Rnetiso << " SLOPE*Rnetiso=" << SLOPE*Rnetiso <<  " 1000*VPDa*HDIVT=" << 1000.0*VPDa*HDIVT << " VPDa=" << VPDa << " HDIVT=" << HDIVT << " GAMMA*GH/GV=" << GAMMA*GH/GV << endl;
    // }
...
leafFluxes Tree::Photosyn(Context &ctx, float PPFD, float TLEAF, float CS, float DS)
{
    ...
    {
        LOG_COUT << "Warning in Photosyn ! GSDIVA=" << GSDIVA << "; DS=" << DS << "; CS=" << CS << endl;
        LOG_COUT << "In Photosyn: PPFD=" << PPFD << "; TLEAF=" << TLEAF << "; CS=" << CS << "; DS=" << DS << endl;
    }
    ...
    {
        LOG_COUT << "Warning in Photosyn: " << ", Anet= " << Anet << ", GSDIVA=" << GSDIVA << "; Anet*GSDIVA=" << Anet * GSDIVA << ", AC= " << AC << ", AJ= " << AJ << ", Rday=" << Rday << ", CS=" << CS << "; CIC=" << CIC << "; CIJ=" << CIJ << ", DS= " << DS << ", TLEAF= " << TLEAF << ", t_WSF_A=" << t_WSF_A << ", t_WSF= " << t_WSF << "; GammaT=" << GammaT << "; KmT=" << KmT << "; J=" << J << "; PPFD=" << PPFD << "; T_Rdark=" << t_Rdark << "; convT=" << convT << endl;
    }
    ...
    {
        LOG_COUT << "Warning in Photosyn: " << ", Anet= " << Anet << ", AC= " << AC << ", AJ= " << AJ << ", Rday=" << Rday << ", GSDIVA=" << GSDIVA << ", CS=" << CS << "; CIC=" << CIC << "; CIJ=" << CIJ << ", DS= " << DS << ", TLEAF= " << TLEAF << ", t_WSF_A=" << t_WSF_A << ", t_WSF= " << t_WSF << "; GammaT=" << GammaT << "; KmT=" << KmT << "; J=" << J << "; PPFD=" << PPFD << "; t_Rdark=" << t_Rdark << "; convT=" << convT << endl;
    }
...
leafFluxes Tree::Photosyn(Context &ctx, float PPFD, float TLEAF, float CS, float DS)
{
    ...
    if (isnan(Anet) || isnan(GSDIVA))
    {
        LOG_COUT << "Warning in Photosyn: " << ", Anet= " << Anet << ", GSDIVA=" << GSDIVA << "; Anet*GSDIVA=" << Anet * GSDIVA << ", AC= " << AC << ", AJ= " << AJ << ", Rday=" << Rday << ", CS=" << CS << "; CI=" << CI << ", DS= " << DS << ", TLEAF= " << TLEAF << ", t_WSF_A=" << t_WSF_A << ", t_WSF= " << t_WSF << "; GammaT=" << GammaT << "; KmT=" << KmT << "; J=" << J << "; PPFD=" << PPFD << "; T_Rdark=" << t_Rdark << "; convT=" << convT << endl;
    }
    ...
    if (isnan(outPhotosyn.water_flux) || isnan(outPhotosyn.carbon_flux))
    {
        LOG_COUT << "Warning in Photosyn: " << ", Anet= " << Anet << ", AC= " << AC << ", AJ= " << AJ << ", Rday=" << Rday << ", GSDIVA=" << GSDIVA << ", CS=" << CS << "; CI=" << CI << ", DS= " << DS << ", TLEAF= " << TLEAF << ", t_WSF_A=" << t_WSF_A << ", t_WSF= " << t_WSF << "; GammaT=" << GammaT << "; KmT=" << KmT << "; J=" << J << "; PPFD=" << PPFD << "; T_Rdark=" << t_Rdark << "; convT=" << convT << endl;
    }
...
leafFluxes Tree::dailyFluxesLeaf(Context &ctx, float PPFD, float VPD, float T, float W, float ExtinctLW, float PPFDinc, float &wf_1016)
{
    ...
    // LOG_COUT << "In dailyFluxesLeaf PPFD=" << PPFD << "; T=" << T << "; VPD=" << VPD << "; W=" << W << endl;
    ...
    for (int i = 0; i < ctx.time.nbsteps_varday; i++)
    {
        ...
        {
            LOG_COUT << "Warning in dailyFluxesLeaf !!! vpd_wardaytimestep <=0" << endl;
        }
        if (ppfd_vardaytimestep <= 0)
        {
            LOG_COUT << "Warning ppfd_vardaytimestep  <=0 in dailyFluxesLeaf !!!" << endl;
        }
        ...
        if (vpd_vardaytimestep <= 0)
        {
            LOG_COUT << "Warning in dailyFluxesLeaf !!! i=" << i << "; vpd_vardaytimestep=" << vpd_vardaytimestep << "; VPD=" << VPD << "; ctx.climate.varday_vpd[i]=" << ctx.climate.varday_vpd[i] << "; ppfd_vardaytimestep=" << ppfd_vardaytimestep << "; PPFD=" << PPFD << "; ctx.climate.varday_light[i]=" << ctx.climate.varday_light[i] << endl;
        }
        if (ppfd_vardaytimestep <= 0)
        {
            LOG_COUT << "Warning ppfd_vardaytimestep  <=0 in dailyFluxesLeaf !!! ppfd_vardaytimestep=" << ppfd_vardaytimestep << "; PPFD=" << PPFD << "; ctx.climate.varday_light[i]=" << ctx.climate.varday_light[i] << endl;
        }
        ...
        {
            LOG_COUT << "Warning in dailyFluxesLeaf: " << " fluxes_vardaytimestep.carbon_flux= " << fluxes_vardaytimestep.carbon_flux << ", fluxes_vardaytimestep.water_flux= " << fluxes_vardaytimestep.water_flux << ", dailyA=" << dailyA << ", dailylT=" << dailylT << ", i=" << i << endl;
        }

        // LOG_COUT << dailyA << endl;
    ...
    // LOG_COUT << "In dailyFluxesLeaf, final daily average: dailyA=" << dailyA << "; dailylT=" << dailylT << endl;
    ...
    {
        LOG_COUT << "Warning in dailyFluxesLeaf:" << " dailyA=" << dailyA << ", dailylT=" << dailylT << ", PPFD=" << PPFD << endl;
    }
...
float Tree::GPPleaf(Context &ctx, float PPFD, float VPD, float T)
{
    ...
    // if(convT>500 || isnan(convT) || convT <0) LOG_COUT << t_site << " | convT: " << convT << " | T: " << T << " | PPFD: " << PPFD << " | VPD: " << VPD << endl;
...
float Tree::dailyGPPleaf(Context &ctx, float PPFD, float VPD, float T)
{
    ...
    for (int i = 0; i < ctx.time.nbsteps_varday; i++)
    {
        // LOG_COUT << t_site << " i: " << i << " tempRday: " << tempRday << endl;
        float ppfd_vardaytimestep = PPFD * ctx.climate.varday_light[i];
        float vpd_vardaytimestep = VPD * ctx.climate.varday_vpd[i];
        float t_vardaytimestep = T * ctx.climate.varday_T[i];
        if (ppfd_vardaytimestep > 0.1)
            dailyA += Tree::GPPleaf(ctx, ppfd_vardaytimestep, vpd_vardaytimestep, t_vardaytimestep);
        // else { LOG_COUT << endl << t_site << " species: " << t_s->s_name << " t_age: " << t_age << " PPFD: " << ppfd_vardaytimestep << " vpd_vardaytimestep " << vpd_vardaytimestep << " t_vardaytimestep: " << t_vardaytimestep << " GPPleaf: " << Tree::GPPleaf(ctx, ppfd_vardaytimestep,vpd_vardaytimestep,t_vardaytimestep) << endl;}
        //  deprecated in v.2.4.1: compute GPP only if enough light is available threshold is arbitrary, but set to be low: in full sunlight ppfd is aroung 700 W/m2, and even at dawn, it is ca 3% of the max value, or 20 W/m2. The minimum threshold is set to 0.1 W/m2
        //  Future update: compute slightly more efficiently, using 3-hourly values? This will have to be aligned with climate forcing layers (e.g. NCAR)
...
float Tree::Rdayleaf(Context &ctx, float T)
{
    ...
    // if(T < 0 || isnan(T) || T > 50) LOG_COUT << t_site << " species: " << t_s->s_name << " convT: " << T << endl;
...
void Tree::CalcRespGPP(Context &ctx)
{
    ...
    if (t_LA > 0.0)
    {
        ...
        for (int h = crown_above_top; h >= h_stop; h--)
        {
            ...
            if (W <= 0)
            {
                LOG_COUT << " Wind=" << W << " h=" << h << " MeanDCELLHeight=" << MeanDCELLHeight << " convHratio=" << convHratio << " ctx.lookup.LookUp_Wind[convHratio]=" << ctx.lookup.LookUp_Wind[convHratio] << endl;
            }

            // LOG_COUT << " Wind=" << W << " h=" << h << " MeanDCELLHeight=" << MeanDCELLHeight <<" convHratio=" << convHratio << " ctx.lookup.LookUp_Wind[convHratio]=" << ctx.lookup.LookUp_Wind[convHratio] << " ctx.soil.TopWindSpeed_DCELL[ctx.grid.site_DCELL[t_site]]=" << ctx.soil.TopWindSpeed_DCELL[ctx.grid.site_DCELL[t_site]] << endl;
            ...
            {
                // LOG_COUT << "Rounding when h=" <<h << " with crown_above_top=" << crown_above_top << " and leafarea_toprounding=" << leafarea_toprounding << endl;
            ...
            {
                LOG_COUT << "Warning leafarea <=0 in CalcRespGPP !! PPFD=" << PPFD << "; leafarea_layer=" << leafarea_layer << " when h=" << h << " with crown_above_top=" << crown_above_top << " and leafarea_toprounding=" << leafarea_toprounding << endl;
            }
            ...
            if (leafarea_layer > 0.0 && PPFD > 0)
            {
                if (PPFD < 0)
                {
                    LOG_COUT << "Warning PPFD <0 in CalcRespGPP !! PPFD=" << PPFD << endl;
                }
                ...
                if (isnan(t_GPP) || isnan(t_transpiration) || t_transpiration < 0.0)
                    LOG_COUT << "Problem at site: " << t_site << ", t_GPP= " << t_GPP << ", transpiration= " << t_transpiration << ", leafarea_layer= " << leafarea_layer << ", PPFD=" << PPFD << ", VPD= " << VPD << ", T= " << Tmp << ", t_WSF_A=" << t_WSF_A << ", t_WSF= " << t_WSF << endl;

#else
                t_GPP += leafarea_layer * Tree::dailyGPPleaf(ctx, PPFD, VPD, Tmp);
                // if(isnan(t_GPP) || t_GPP < 0.0 || PPFD == 0.0) LOG_COUT << "Problem at site: " <<  t_site << " leafarea_layer: " << leafarea_layer << " GPP: " << t_GPP << " PPFD: " << PPFD << " VPD: " << VPD << " T: " << Tmp << endl << endl;
#endif
                ...
                // if(t_dbh > 0.2) LOG_COUT << crown_above_top - h << "PPFD: " << PPFD << " VPD: " << VPD << " Tmp: " << Tmp << " leafarea_layer: " << endl;
...
void Tree::UpdateLeafDynamics(Context &ctx)
{
    ...
    else
    {
        ...
        for (int i = 0; i < ctx.grid.leafdem_resolution; i++)
        {
            ...
            if (new_litter < 0)
            {
                LOG_COUT << "Warning in UpdateLeafDynamics, newlitter <0, lambda_old=" << lambda_old << " t_oldLA=" << t_oldLA << " t_pheno_factor=" << t_pheno_factor << endl;
            }
            ...
            // LOG_COUT << "flush_needed=" << flush_needed << " t_LAmax=" << t_LAmax << " t_LA=" << t_LA << " new_litter=" << new_litter << " new_young=" << new_young << endl;
            ...
            {
                LOG_COUT << "Warning in UpdateLeafDynamics, new_litter=" << new_litter << ", lambda_old=" << lambda_old << " t_oldLA=" << t_oldLA << " t_pheno_factor=" << t_pheno_factor << " t_LMA=" << t_LMA << " new_old=" << new_old << " t_Ndays_dry=" << t_Ndays_dry << " t_Ndays_wet=" << t_Ndays_wet << endl;
            }
...
{
    LOG_COUT << ctx.time.iter << "\t" << t_site << "\t" << t_sp_lab << "\t" << t_height << "\t" << t_dbh << "\t" << t_litter << "\t" << t_age << "\t" << t_LA << "\t" << t_youngLA << "\t" << t_matureLA << "\t" << t_oldLA << "\t" << t_CR << "\t" << t_CD << "\t" << t_GPP << "\t" << t_NPP << "\t" << t_Rstem << "\t" << t_Rday << "\t" << t_Rnight << "\t" << ctx.field.LAI3D[int(t_height)][t_site + ctx.grid.SBORD] << "\t" << ctx.field.LAI3D[int(t_height - t_CD) + 1][t_site + ctx.grid.SBORD] << "\t" << t_root_depth << "\t" << t_phi_root << "\t" << t_WSF;
    for (int l = 0; l < ctx.soil.nblayers_soil; l++)
        LOG_COUT << "\t" << t_root_biomass[l];
    for (int l = 0; l < ctx.soil.nblayers_soil; l++)
        LOG_COUT << "\t" << t_soil_layer_weight[l];
    LOG_COUT << endl;
}
