...
void Initialise(Context &ctx)
{
    ...
    for (int site = 0; site < ctx.grid.sites; site++)
    {
        ...
        {
            LOG_COUT << "Warning mem_alloc root biomass and soil ctx.diag.layer weight at site " << site << ": " << endl;
            LOG_COUT << "t_soil_layer_weight adresses: " << &T_site.t_soil_layer_weight[0] << "\t" << &T_site.t_soil_layer_weight[1] << "\t" << &T_site.t_soil_layer_weight[2] << "\t" << &T_site.t_soil_layer_weight[3] << "\t" << &T_site.t_soil_layer_weight[4] << endl;
            LOG_COUT << "t_root biomass adresses: " << &T_site.t_root_biomass[0] << "\t" << &T_site.t_root_biomass[1] << "\t" << &T_site.t_root_biomass[2] << "\t" << &T_site.t_root_biomass[3] << "\t" << &T_site.t_root_biomass[4] << endl;
        }
#endif
    }
    ...
    {
        std::LOG_COUT << "LUT cache missing → computing LUTs..." << std::endl;
        InitialiseLookUpTables(ctx);
        SaveLookUpTablesToCache(ctx);
    }
    else
    {
        std::LOG_COUT << "Loaded LUTs from cache." << std::endl;
    }
#else  // CACHE_LUT
    InitialiseLookUpTables(ctx);
#endif // CACHE_LUT
...
void InitialiseABC(Context &ctx)
{
    ...
    LOG_COUT << "row start: " << row_start << " | row end: " << row_end << " | sites_abc: " << sites_abc << endl;
...
void ReadInputInventory(Context &ctx)
{
    ...
    if (InInventory)
    {
        ...
        if (flag_dbh == 1)
        {
            LOG_COUT << "WARNING! No diameter column provided, no initialization from file will be carried out." << endl;
        }
        else
        {
            if ((flag_col == 1) | (flag_row == 1))
                LOG_COUT << "WARNING! At least one coordinate column (col/row) is missing, random coordinates chosen" << endl;
            if (flag_species == 1)
                LOG_COUT << "WARNING! No species column provided, random species chosen" << endl;
            ...
            {
                LOG_COUT << "Successfully initialised " << nb_individuals << " out of " << nb_parameterlines << " trees from file." << endl;
                LOG_COUT << "Coordinates: " << nb_moved << " trees were moved due to overlapping coordinates, and " << nb_random << " were placed randomly on the grid as coordinates were incomplete or could not be read in." << endl;
                LOG_COUT << "Species: " << nb_speciesrandom << " trees were assigned a random species." << endl;
                if (sites_shuffled_index == ctx.grid.sites)
                    LOG_COUT << "WARNING: shuffle index: " << sites_shuffled_index << " is equal site number. This means that random placement of trees has stopped early as the algorithm has run out of grid cells to place trees. This may indicate that there were more trees than grid cells." << endl;
            }
            else
            {
                LOG_COUT << "WARNING! Inventory file was empty. No trees were initialised." << endl;
            }
        }
    }
    else
    {
        LOG_COUT << "ERROR with the inventory file" << endl;
    }
    ...
    if (InSWC)
    {
        ...
        while (getline(InSWC, line))
        {
            ...
            if (l != ctx.soil.nblayers_soil)
                LOG_COUT << "Error in SWC input file (number of layers for dcell " << dcell << endl;
            l = 0;
            dcell++;
        }

        if (dcell != ctx.grid.nbdcells)
            LOG_COUT << "Error in SWC input file (number of dcells) " << endl;
    }
    else
    {
        LOG_COUT << "ERROR with the input SWC file" << endl;
    }
        ...
        for (int l = 0; l < ctx.soil.nblayers_soil; l++)
        {
            ...
            if (ctx.opt._WATER_RETENTION_CURVE == 1)
            {
                if (theta_w == 0)
                {
                    ...
                    LOG_COUT << "Warning theta_w = 0 " << endl;
                }
                ...
                if (isnan(ctx.soil.soil_phi3D[l][d]) || isnan(ctx.soil.Ks[l][d]) || (ctx.soil.SWC3D[l][d] - ctx.soil.Min_SWC[l]) < 0) //|| ctx.soil.KsPhi[l][d]==0.0 || ctx.soil.Ks[l][d]==0.0 || ctx.soil.soil_phi3D[l][d]==0.0)
                    LOG_COUT << "In bucket model, ctx.diag.layer " << l << " dcell " << d << " theta_w=" << theta_w << " ctx.soil.SWC3D[l][d]-ctx.soil.Min_SWC[l]=" << (ctx.soil.SWC3D[l][d] - ctx.soil.Min_SWC[l]) << " ctx.soil.soil_phi3D[l][d]=" << ctx.soil.soil_phi3D[l][d] << " ctx.soil.Ksat=" << ctx.soil.Ksat[l] << " ctx.soil.Ks[l][d]=" << ctx.soil.Ks[l][d] << endl;
            }
            else if (ctx.opt._WATER_RETENTION_CURVE == 0)
            {
    ...
    //        //LOG_COUT << site << " Site of tree: " << ctx.T[site].t_site << " Height: " << ctx.T[site].t_height << " Height from index: " << heights_trees[index_site] << endl;
...
void AllocMem(Context &ctx)
{
    ...
    LOG_COUT << "ctx.grid.SBORD: " << ctx.grid.SBORD << endl;
    if (!mpi_rank)
    {
        // LOG_COUT << "ctx.grid.HEIGHT : " << ctx.grid.HEIGHT << " ctx.grid.RMAX : " << ctx.grid.RMAX << " DBH : " << DBH <<"\n"; LOG_COUT.flush();
        if (ctx.grid.RMAX > ctx.grid.rows)
        {
            // Consistency tests
            LOG_CERR << "Error : ctx.grid.RMAX > ctx.grid.rows \n";
            exit(-1);
        }
        if (ctx.grid.HEIGHT > ctx.grid.rows)
        {
            LOG_CERR << "Error : ctx.grid.HEIGHT > ctx.grid.rows \n";
            exit(-1);
        }
    }
    ...
    for (int spp = 1; spp <= ctx.grid.nbspp; spp++)
    {
        ...
        // LOG_COUT << "prob_species: " << prob_species << endl;
    ...
    if (ctx.opt._NDD)
        if (NULL == (ctx.species.PROB_S = new float[ctx.grid.nbspp + 1]))
            LOG_CERR << "!!! Mem_Alloc\n";
    //  if (NULL==(persist=new long int[ctx.time.nbiter])) LOG_CERR<<"!!! Mem_Alloc\n";                  // Field for persistence
    //  if (NULL==(distr=new int[ctx.grid.cols])) LOG_CERR<<"!!! Mem_Alloc\n";
    ...
    if (NULL == (ctx.soil.KsPhi = new float *[ctx.soil.nblayers_soil]))
        LOG_CERR << "!!! Mem_Alloc\n";
    // if (NULL==(KsPhi2=new float*[ctx.soil.nblayers_soil])) LOG_CERR<<"!!! Mem_Alloc\n";
    ...
    for (int l = 0; l < ctx.soil.nblayers_soil; l++)
    {
        ...
        if (NULL == (ctx.soil.KsPhi[l] = new float[ctx.grid.nbdcells]))
            LOG_CERR << "!!! Mem_Alloc\n";
        // if (NULL==(KsPhi2[l]=new float[ctx.grid.nbdcells])) LOG_CERR<<"!!! Mem_Alloc\n";
        ...
        for (int dcell = 0; dcell < ctx.grid.nbdcells; dcell++)
        {
            ...
            {
                LOG_COUT << ctx.soil.SWC3D[l][dcell] << "\t" << ctx.soil.Max_SWC[l] << "\n";
            }
        }
    }
