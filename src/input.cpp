...
void AssignSpeciesParam(Species &S, const std::string &name, const std::string &value)
{
    ...
    {
        LOG_CERR << "Warning: Unknown species parameter '" << name << "'\n";
        return;
    }
...
void ReadInputGeneral(Context &ctx)
{
    ...
    if (In)
    {
        ...
        {
            LOG_COUT << "ERROR. General input file could not be read." << endl;
        }
        ...
        LOG_COUT << "ctx.grid.rows: " << ctx.grid.rows << " ctx.grid.cols: " << ctx.grid.cols << " ctx.grid.HEIGHT: " << ctx.grid.HEIGHT << endl;
        LOG_COUT << "Number of dcells: " << ctx.grid.nbdcells << endl;
        LOG_COUT << "Lin number of dcells: " << ctx.grid.linear_nb_dcells << endl;
        LOG_COUT << ctx.opt._WATER_RETENTION_CURVE << " " << ctx.opt._SOIL_LAYER_WEIGHT << " " << ctx.params.Cair << endl;
        ...
        {
            LOG_COUT << "Correlation status. ctx.intra.corr_N_P: " << ctx.intra.corr_N_P << " ctx.intra.cov_N_LMA: " << ctx.intra.corr_N_LMA << " ctx.intra.corr_P_LMA: " << ctx.intra.corr_P_LMA << endl;
            ...
            for (int mrow = 0; mrow < 3; mrow++)
                LOG_COUT << gsl_matrix_get(ctx.rng.mcov_N_P_LMA, mrow, 0) << "\t" << gsl_matrix_get(ctx.rng.mcov_N_P_LMA, mrow, 1) << "\t" << gsl_matrix_get(ctx.rng.mcov_N_P_LMA, mrow, 2) << endl;
...
void ReadInputSpecies(Context &ctx)
{
    ...
    if (InSpecies)
    {
        ...
        while (firstlinestream >> parameter_name)
        {
            ...
            if (index == -1)
            {
                LOG_COUT << "Ignoring unknown parameter: " << parameter_name << ". Parameters must be one of:";
                for (int i = 0; i < nb_parameters; i++)
                {
                    LOG_COUT << "\t" << parameter_names[i];
                }
                LOG_COUT << endl;
            }
        ...
        if (nb_parameterlines > 0)
        {
            ...
            LOG_COUT << "Successfully initialised " << ctx.grid.nbspp << " species from file." << endl;
        }
        else
        {
            ctx.grid.nbspp = 1;
            LOG_COUT << "WARNING! Species file was empty. A default species is initialized." << endl;
            vector<string> parameter_values(nb_parameters, "");
        ...
        for (int sp = 1; sp <= ctx.grid.nbspp; sp++)
        {
            ...
            ctx.S[sp].Init(ctx);
            // LOG_COUT << ctx.S[sp].s_name << " LCP: " << ctx.S[sp].s_LCP << endl;
        }
        LOG_COUT << "Successfully read in species file." << endl;
    }
    else
    {
        LOG_COUT << "ERROR with the species file" << endl;
    }
    ...
    if (InDaily)
    {
        ...
        while (getline(InDaily, line))
        {
            ...
            if (varday_vpd_current <= 0.01)
            {
                ...
                LOG_COUT << "Warning, VPD is <=0.01, on day " << day_current << " at " << starttime_current << "h." << endl;
            }

            if (varday_light_current <= 1)
            {
                varday_light_current = 1;
                LOG_COUT << "Warning, light is <=1, on day " << day_current << " at " << starttime_current << "h." << endl;
            }

            if (varday_WS_current <= 0.1)
            {
                varday_WS_current = 0.1;
                LOG_COUT << "Warning, wind is <=0.1, on day " << day_current << " at " << starttime_current << "h." << endl;
            }
            ...
            // LOG_COUT << "at: " << starttime_current << " vardaytime_light: " << ctx.climate.varday_light[ctx.time.nbsteps_varday] << " vardaytime_vpd: " << ctx.climate.varday_vpd[ctx.time.nbsteps_varday] << " vardaytime_T: "<< ctx.climate.varday_T[ctx.time.nbsteps_varday] << " vardaytime_WS: "<< ctx.climate.varday_WS[ctx.time.nbsteps_varday] << endl;
            ctx.time.nbsteps_varday++;
        }

        ctx.time.nbsteps_varday /= ctx.time.nbdays;

        LOG_COUT << "Read in: " << ctx.time.nbsteps_varday << " timesteps per day, covering " << ctx.time.nbhours_covered << " hours of the day." << endl;
        LOG_COUT << "Successfully read in daytime variation file" << endl;
        ctx.time.inv_nbsteps_varday = 1.0 / float(ctx.time.nbsteps_varday);
    }
    else
    {
        LOG_COUT << "ERROR with the daily variation file" << endl;
    }

    if (NULL == (ctx.climate.WDailyMean_all = new float[ctx.time.nbsteps_varday]))
        LOG_CERR << "!!! ctx.climate.WDailyMean_all" << endl;
    if (NULL == (ctx.climate.tDailyMean_all = new float[ctx.time.nbsteps_varday]))
        LOG_CERR << "!!! ctx.climate.tDailyMean_all" << endl;
    if (NULL == (ctx.climate.VPDDailyMean_all = new float[ctx.time.nbsteps_varday]))
        LOG_CERR << "!!! ctx.climate.VPDDailyMean_all" << endl;
    if (NULL == (ctx.climate.windDailyMean_all = new float[ctx.time.nbsteps_varday]))
        LOG_CERR << "!!! ctx.climate.windDailyMean_all" << endl;
...
void ReadInputClimate(Context &ctx)
{
    ...
    if (InClim)
    {
        ...
        // LOG_COUT << "Header line: " << ctx.fileio.buffer << endl;
        ...
        LOG_COUT << "Read in climate data for " << ctx.time.nbdays << " days, with" << ctx.time.iterperyear << " iterations per year." << endl;
        ...
        LOG_COUT << "Successfully read the climate file" << endl;
    }
    else
    {
        LOG_COUT << "ERROR with the climate file" << endl;
    }
    InClim.close();
}
...
void ReadInputDailyvar(Context &ctx)
{
    ...
    if (InDaily)
    {
        ...
        for (int i = 0; i < ctx.time.nbsteps_varday; i++)
        {
            ...
            LOG_COUT << i << "\t" << ctx.climate.varday_light[i] << endl;
        }
        LOG_COUT << totday_light << endl;
...
void ReadInputClimate(Context &ctx)
{
    ...
    if (InClim)
    {
        ...
        while (getline(InClim, line))
        {
            ...
            ctx.climate.DailyMeanIrradiance.push_back(DailyMeanIrradiance_current);
            ctx.climate.DailyMeanVapourPressureDeficit.push_back(DailyMeanVapourPressureDeficit_current);
            // LOG_COUT <<  DailyMeanTemperature_current << "\t" <<  NightTemperature_current << "\t" <<  Rainfall_current << "\t" <<  DailyMeanWindSpeed_current << "\t" <<  DailyMeanIrradiance_current << "\t" <<   DailyMeanVapourPressureDeficit_current << endl;
            ctx.time.iterperyear++;
        }

        ctx.time.timestep = 1.0 / float(ctx.time.iterperyear);
        LOG_COUT << "Read in climate data for " << ctx.time.iterperyear << " iterations per year." << endl;
        // choose average conditions
...
#endif
        // LOG_COUT << "ctx.climate.WDailyMean: " << ctx.climate.WDailyMean_year << endl;
...
void ReadInputSoil(Context &ctx)
{
    ...
    if (InSoil)
    {
        ...
        LOG_COUT << "Read in: " << ctx.soil.nblayers_soil << " soil layers" << endl;

        if (NULL == (ctx.soil.layer_depth = new float[ctx.soil.nblayers_soil]))
            LOG_CERR << "!!! Mem_Alloc ctx.soil.layer_depth" << endl;
        ...
        for (int l = 0; l < ctx.soil.nblayers_soil; l++)
        {
            ...
            LOG_COUT << "ctx.diag.layer " << l << " Vol=" << ctx.grid.sites_per_dcell * ctx.grid.LH * ctx.grid.LH * layer_thickness[l] << " m3; ctx.soil.Sat_SWC =" << ctx.soil.Sat_SWC[l] << " MAX_SWC =" << ctx.soil.Max_SWC[l] << " m3." << endl;
        }

        if (NULL == (ctx.soil.Ksat = new float[ctx.soil.nblayers_soil]))
            LOG_CERR << "!!! Mem_Alloc ctx.soil.Ksat" << endl;
        for (int l = 0; l < ctx.soil.nblayers_soil; l++)
        {
            ...
            LOG_COUT << "ctx.diag.layer " << l << " ctx.soil.Ksat=" << ctx.soil.Ksat[l] << "mm/s or kg/m2/s  " << ctx.soil.Ksat[l] * 9.8 / 18 << endl;
        }

        if (NULL == (ctx.soil.Res_SWC = new float[ctx.soil.nblayers_soil]))
            LOG_CERR << "!!! Mem_Alloc Res_SW" << endl;
        ...
        for (int l = 0; l < ctx.soil.nblayers_soil; l++)
        {
            ...
            LOG_COUT << "ctx.diag.layer " << l << " Vol=" << ctx.grid.sites_per_dcell * ctx.grid.LH * ctx.grid.LH * layer_thickness[l] << "m3; Res=" << ctx.soil.Res_SWC[l] << " MIN_SWC =" << ctx.soil.Min_SWC[l] << " m3" << endl;
        }

        if (ctx.opt._WATER_RETENTION_CURVE == 1)
        {
            if (NULL == (ctx.soil.a_vgm = new float[ctx.soil.nblayers_soil]))
                LOG_CERR << "!!! Mem_Alloc ctx.soil.a_vgm" << endl;
            if (NULL == (ctx.soil.b_vgm = new float[ctx.soil.nblayers_soil]))
                LOG_CERR << "!!! Mem_Alloc ctx.soil.b_vgm" << endl;
            if (NULL == (ctx.soil.c_vgm = new float[ctx.soil.nblayers_soil]))
                LOG_CERR << "!!! Mem_Alloc ctx.soil.c_vgm" << endl;
            if (NULL == (ctx.soil.m_vgm = new float[ctx.soil.nblayers_soil]))
                LOG_CERR << "!!! Mem_Alloc ctx.soil.m_vgm" << endl;
            ...
            for (int l = 0; l < ctx.soil.nblayers_soil; l++)
            {
                ...
                LOG_COUT << "ctx.diag.layer " << l << " alpha_vgm=" << alpha_vgm << "\t" << "n_vgm=" << n_vgm << " ctx.soil.FC_SWC=" << ctx.soil.FC_SWC[l] << endl;
            }
        }
        else if (ctx.opt._WATER_RETENTION_CURVE == 0)
        {
            if (NULL == (ctx.soil.phi_e = new float[ctx.soil.nblayers_soil]))
                LOG_CERR << "!!! Mem_Alloc ctx.soil.phi_e" << endl;
            if (NULL == (ctx.soil.b = new float[ctx.soil.nblayers_soil]))
                LOG_CERR << "!!! Mem_Alloc b" << endl;
            for (int l = 0; l < ctx.soil.nblayers_soil; l++)
            {
                ...
                LOG_COUT << "ctx.diag.layer " << l << " ctx.soil.phi_e=" << ctx.soil.phi_e[l] << "\t" << "b=" << ctx.soil.b[l] << " ctx.soil.FC_SWC=" << ctx.soil.FC_SWC[l] << endl;
            }
        }
        LOG_COUT << "Successfully read the soil file" << endl;
    }
    else
    {
        LOG_COUT << "ERROR with the soil file" << endl;
    }
    LOG_COUT << endl;
}
#endif
