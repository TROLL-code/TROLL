...
void Average(Context &ctx)
{
    ...
    if (!mpi_rank)
    {
        ...
        LOG_COUT.setf(ios::fixed, ios::floatfield);
        LOG_COUT.precision(2);

#ifdef WATER
        LOG_COUT << ctx.time.iter << "\tTrees (1/ha): " << sum1 << " | " << sum10 << " | " << sum30 << " *** nbdead (%): " << 100.0 * ctx.diag.nbdead_n1 * inbhectares / sum1 << " | " << 100.0 * ctx.diag.nbdead_n10 * inbhectares / sum10 << " | " << 100.0 * ctx.diag.nbdead_n30 * inbhectares / sum30 << " *** AGB (t/ha): " << round(agb / 1000.0) << " GPP (MgC/ha/yr) " << gpp * ctx.time.iterperyear << " NPP " << npp * ctx.time.iterperyear << " litterfall (Mg/ha/yr) " << litterfall * ctx.time.iterperyear << " *** ctx.soil.Transpiration (mm): ";
#else

        LOG_COUT << ctx.time.iter << "\tTrees (1/ha): " << sum1 << " | " << sum10 << " | " << sum30 << " *** nbdead (%): " << 100.0 * ctx.diag.nbdead_n1 * inbhectares / sum1 << " | " << 100.0 * ctx.diag.nbdead_n10 * inbhectares / sum10 << " | " << 100.0 * ctx.diag.nbdead_n30 * inbhectares / sum30 << " *** AGB (t/ha): " << round(agb / 1000.0) << " GPP (MgC/ha/yr) " << gpp * ctx.time.iterperyear << " NPP " << npp * ctx.time.iterperyear << " litterfall (Mg/ha/yr) " << litterfall * ctx.time.iterperyear << endl;
#endif
    ...
    LOG_COUT.setf(ios::fixed, ios::floatfield);
    LOG_COUT.precision(5);

    LOG_COUT << ctx.time.iter << "\tTrunkC: " << carbon_stored_trunk * factor_weight << " LeavesC: " << carbon_stored_leaves * factor_weight << " FreeC: " << carbon_stored_free * factor_weight << " Total AssimC: " << carbon_assimilated_total * factor_weight << " Total NetC: " << carbon_net_total * factor_weight << endl;
    LOG_COUT << ctx.time.iter << "\tTrunkC change: " << (carbon_stored_trunk - carbon_stored_trunk_previous) * factor_weight << " LeavesC: " << (carbon_stored_leaves - carbon_stored_leaves_previous) * factor_weight << " FreeC: " << (carbon_stored_free - carbon_stored_free_previous) * factor_weight << endl;
    ...
    LOG_COUT << ctx.soil.transpiration_1016 * 1000 << " | " << endl;
...
{
    LOG_COUT << "Writing snapshot of forest to file." << endl;
    if (header == 1)
    {
...
//        std::LOG_COUT << "big-endian\n";
//    else if constexpr (std::endian::native == std::endian::little)
//        std::LOG_COUT << "little-endian\n";
//    else std::LOG_COUT << "mixed-endian\n";
//}
...
/* void ExportPointcloudHeader(vector<int> &beams, fstream& ctx.out.output_pointcloud){
    ...
    LOG_COUT << "Sampled " << nb_beams << " pulses, creating " << nb_returns << " returns." << endl;
 ...
 void ExportPointcloud(float mean_beam, float sd_beam, float klaser, float ctx.pc.transmittance_laser, fstream& ctx.out.output_pointcloud){
    LOG_COUT << "Point cloud generation." << endl;
