...
void UpdateSeeds(Context &ctx)
{
    ...
    if (ctx.time.iter % ctx.time.iterperyear == 0)
    {
        ...
        LOG_COUT << ctx.grid.sites << " Seedrain: " << ctx.params.Cseedrain * ha << endl;
        int seedsadded = 0;
        for (int s = 0; s < ctx.grid.sites; s++)
        {
            // if(ctx.T[s].t_age == 0){
            int nbseeds = ctx.species.n_seed[s];
            // LOG_COUT << "Site: " << s << " nbseeds: " << nbseeds << " ctx.grid.nbspp: " << ctx.grid.nbspp << endl;
            ...
            for (int spp = 1; spp <= ctx.grid.nbspp; spp++)
            {
                ...
                // LOG_COUT << "Site: " << s << " Species: " << spp << " nbseeds: " << nbseeds_species << endl;
        ...
        LOG_COUT << "Trees_mature: " << trees_mature << " Nbseedsadded: " << seedsadded << " effective: " << seedsadded_effective << " nbspeciesaffected: " << nbspecies_affected << endl;
    }
}
...
void UpdateField(Context &ctx)
{
    ...
    for (int d = 0; d < ctx.grid.nbdcells; d++)
    {
        ...
        {
            LOG_COUT << "in UpdateField: d=" << d << "; Canopyheight_DCELL[d]=" << ctx.soil.Canopy_height_DCELL[d] << "; ctx.soil.HSum_DCELL[d]=" << ctx.soil.HSum_DCELL[d] << "; ctx.soil.TopWindSpeed_DCELL[d]=" << ctx.soil.TopWindSpeed_DCELL[d] << endl;
        }
#else
        if (ctx.soil.Canopy_height_DCELL[d] <= MeteoStation_Height)
        {
        ...
        // LOG_COUT << "iter=" << ctx.time.iter << " ctx.climate.WSDailyMean=" << ctx.climate.WSDailyMean << " d=" << d <<  " canopy_height_DCELL[d]=" << ctx.soil.Canopy_height_DCELL[d] << " ctx.soil.HSum_DCELL[d]=" << ctx.soil.HSum_DCELL[d] <<" ctx.soil.TopWindSpeed_DCELL[d]=" << ctx.soil.TopWindSpeed_DCELL[d];
        // if (d==225) LOG_COUT << " d=" << d <<  " canopy_height_DCELL[d]=" << ctx.soil.Canopy_height_DCELL[d] ;
        // LOG_COUT << endl;
    }
#endif
    ...
    for (int d = 0; d < ctx.grid.nbdcells; d++)
    {
        ...
        for (int l = 0; l < ctx.soil.nblayers_soil; l++)
        {
            ...
            {
                LOG_COUT << "l=" << l << " d= " << d << " transpiration=" << ctx.soil.Transpiration[l][d] << " and ctx.soil.SWC3D[l][d]=" << ctx.soil.SWC3D[l][d] << " and ctx.soil.Min_SWC[l]=" << ctx.soil.Min_SWC[l] << " and ctx.soil.SWC3D[l][d]-ctx.soil.Min_SWC[l]=" << ctx.soil.SWC3D[l][d] - ctx.soil.Min_SWC[l] << endl;
            }
            ctx.soil.SWC3D[l][d] -= w_uptake;
            ctx.soil.SWC3D[l][d] = fmaxf(ctx.soil.SWC3D[l][d], ctx.soil.Min_SWC[l]);
            if (ctx.soil.Transpiration[l][d] < 0.0 || isnan(ctx.soil.Transpiration[l][d]) || isnan(w_uptake) || (ctx.soil.SWC3D[l][d] - ctx.soil.Min_SWC[l]) < 0)
            {
                LOG_COUT << "After w_uptake, l=" << l << " d= " << d << " transpiration=" << ctx.soil.Transpiration[l][d] << " and ctx.soil.SWC3D[l][d]=" << ctx.soil.SWC3D[l][d] << " and ctx.soil.Min_SWC[l]=" << ctx.soil.Min_SWC[l] << " and ctx.soil.SWC3D[l][d]-ctx.soil.Min_SWC[l]=" << ctx.soil.SWC3D[l][d] - ctx.soil.Min_SWC[l] << endl;
            }
        }
        ...
        {
            LOG_COUT << "evaporation=" << ctx.soil.Evaporation[d] << " and e=" << e << " and ctx.soil.SWC3D[0][d]=" << ctx.soil.SWC3D[0][d] << " and ctx.soil.Min_SWC[0]=" << ctx.soil.Min_SWC[0] << "and ctx.soil.SWC3D[0][d]-ctx.soil.Min_SWC[0]=" << ctx.soil.SWC3D[0][d] - ctx.soil.Min_SWC[0] << "; raero=" << r_aero << "; ctx.soil.Canopy_height_DCELL[d]=" << ctx.soil.Canopy_height_DCELL[d] << "; ctx.soil.TopWindSpeed_DCELL[d]=" << ctx.soil.TopWindSpeed_DCELL[d] << endl;
        }
        ...
        {
            LOG_COUT << "Incorrect throughfall" << endl;
            LOG_COUT << ctx.climate.precip << "\t" << ctx.soil.Interception[d] << "\t" << ctx.soil.LAI_DCELL[0][d] << endl;
        }
            ...
            while((l<ctx.soil.nblayers_soil) && (in>0.0)) {
                if(in>(ctx.soil.Max_SWC[l]-ctx.soil.SWC3D[l][d])) {
                    ...
                    if(isnan(ctx.soil.SWC3D[l][d]) || (ctx.soil.SWC3D[l][d]-ctx.soil.Min_SWC[l])<=0) {
                        LOG_COUT << "incorrect ctx.soil.SWC3D, Min/ctx.soil.Max_SWC" << endl;
                        LOG_COUT <<ctx.soil.Max_SWC[l] << endl;
                    }
                }
                else{
                    ...
                    if (isnan(ctx.soil.SWC3D[l][d]) || (ctx.soil.SWC3D[l][d]-ctx.soil.Min_SWC[l])<0) {
                        LOG_COUT << "incorrect ctx.soil.SWC3D, Min/ctx.soil.Max_SWC" << endl;
                        LOG_COUT << ctx.soil.Throughfall[d] << "\t" <<in <<"\t" <<  ctx.climate.precip << "\t" << ctx.soil.Interception[d] << "\t" << ctx.soil.LAI_DCELL[0][d] << endl;
                    }
                    in=0.0;
                }
                l++;
...
void FillSeed(Context &ctx, int col, int row, int spp)
{
    if ((col >= 0) && (col < ctx.grid.cols))
    {
        if ((row >= 0) && (row < ctx.grid.rows))
        {
            ...
            // LOG_COUT << "site: " << site << " spp: " << spp << " Seed added!!! " << endl;
            // }
        }
    }
}
